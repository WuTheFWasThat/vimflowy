...
(() ->
  ...
  # NOTE: this section should get moved out, but can't due to
  # unspecified order of registering operators vs subcommands
  ...
  keyDefinitions.registerMotion [CMD_GO, CMD_MARK], {
    description: 'Go to the mark indicated by the cursor, if it exists',
  },  () ->
    return (cursor, options) ->
      do cursor.goMark

  CMD_DELETE = keyDefinitions.commands.DELETE
  keyDefinitions.registerAction [MODES.NORMAL], [CMD_DELETE, CMD_MARK], {
